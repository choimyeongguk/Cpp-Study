...
struct MergeSortTree {
    ll n, INF = 1e18; vvl tree, prefSum;
    MergeSortTree(vl& arr): n(arr.size()), tree(n<<2), prefSum(n<<2) {
        function<void(ll,ll,ll)> init = [&](ll i, ll s, ll e) {
            ...
            prefSum[i].resize(tree[i].size());
            for (ll j=1; j<tree[i].size(); j++)
                prefSum[i][j] = prefSum[i][j-1] + tree[i][j-1];
        };
        init(1, 0, n-1);
    }
    ll query(ll l, ll r, ll mn, ll mx) { return query(1, 0, n-1, l, r, mn, mx); }
    ll query(ll i, ll s, ll e, ll l, ll r, ll mn, ll mx) {
        if (s>r || e<l) return 0;
        if (l<=s && e<=r) {
            ll left = lower_bound(tree[i].begin(), tree[i].end(), mn) - tree[i].begin();
            ll right = upper_bound(tree[i].begin(), tree[i].end(), mx) - tree[i].begin();
            return prefSum[i][right] - prefSum[i][left];
        }
        ll m = (s+e)>>1;
        return query(i<<1, s, m, l, r, mn, mx) + query(i<<1|1, m+1, e, l, r, mn, mx);
    }
};
...
struct HLD {
    ...
    ll queryPath(ll u, ll v, ll l, ll r, MergeSortTree& seg) {
        ll ret = 0;
        while (top[u] != top[v]) {
            if (depth[top[u]] < depth[top[v]]) swap(u, v);
            ret += seg.query(in[top[u]], in[u], l, r);
            u = pa[top[u]];
        }
        if (depth[u] > depth[v]) swap(u, v);
        ret += seg.query(in[u], in[v], l, r);
        return ret;
    }
};
...
void solve(ll testcase){
    ...
    while (Q--) {
        ll a, b, l, r; io >> a >> b >> l >> r;
        io << hld.queryPath(a, b, l, r, seg) << '\n';
    }
}
