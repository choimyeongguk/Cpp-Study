...
void solve(ll testcase){
    ...
    vl good(M); for (auto& e: good) io >> e;
    ll Q; io >> Q;
    ODC odc(N+1, Q);
    for (ll i=0; i<M; i++)
        if (good[i]) odc.addEdge(edge[i].first, edge[i].second, true);
    ll op, I, a, b;
    for (ll i=0; i<Q; i++) {
        io >> op;
        switch (op) {
            case 1:
                io >> I;
                I--;
                if (good[I]) odc.eraseEdge(edge[I].first, edge[I].second);
                else odc.addEdge(edge[I].first, edge[I].second);
                good[I] = !good[I];
                break;
            case 2:
                io >> a >> b;
