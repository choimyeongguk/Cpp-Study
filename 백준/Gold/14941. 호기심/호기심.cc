...
vl prime, prefSumE, prefSumO;   // 짝수/홀수 인덱스만 포함한 prefixSum
...
void preprocess() {
    eratosthenes();
    prefSumE.resize(prime.size()+1);    // prefSumE[i]=j := sum(0~(i-1))=j
    prefSumO.resize(prime.size()+1);
    for (ll i=1; i<=prime.size(); i++) {
        if (i & 1) {
            prefSumO[i] = prefSumO[i-1];
            prefSumE[i] = prime[i-1] + prefSumE[i-1];
        }
        else {
            prefSumO[i] = prime[i-1] + prefSumO[i-1];
            prefSumE[i] = prefSumE[i-1];
        }
    }
}
...
void solve(ll testcase){
    ll a, b; io >> a >> b;
    ll l = lower_bound(prime.begin(), prime.end(), a) - prime.begin();
    ll r = lower_bound(prime.begin(), prime.end(), b) - prime.begin();
    if (prime[r] != b) r--;
    ll ans = l&1 ? 3*(prefSumO[r+1] - prefSumO[l]) - (prefSumE[r+1] - prefSumE[l])
                 : 3*(prefSumE[r+1] - prefSumE[l]) - (prefSumO[r+1] - prefSumO[l]);
    io << ans << '\n';
}
