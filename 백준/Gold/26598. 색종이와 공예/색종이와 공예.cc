...
void solve(ll testcase){
    ll R, C; io >> R >> C;
    vector<string> M(R); for (auto& e: M) io >> e;
    vvl visit(R, vl(C, false));
    vvl d = {{-1, 0, 1, 0}, {0, 1, 0, -1}};
    function<pair<pll,pll>(ll,ll)> dfs = [&](ll r, ll c) -> pair<pll,pll> {
        ll rMin=r, cMin=c, rMax=r, cMax=c;
        visit[r][c] = true;
        for (ll i=0; i<4; i++) {
            ll nr = r + d[0][i];
            ll nc = c + d[1][i];
            if (nr>=0 && nc>=0 && nr<R && nc<C && M[nr][nc]==M[r][c] && !visit[nr][nc]) {
                auto [Min, Max] = dfs(nr, nc);
                rMin = min(rMin, Min.first);
                cMin = min(cMin, Min.second);
                rMax = max(rMax, Max.first);
                cMax = max(cMax, Max.second);
            }
        }
    ...
    for (ll i=0; i<R; i++) {
        for (ll j=0; j<C; j++) {
            ...
            auto [Min, Max] = dfs(i, j);
            for (ll k=Min.first; k<=Max.first; k++) {
                for (ll l=Min.second; l<=Max.second; l++) {
                    if (M[k][l] != M[i][j]) {
                        io << "BaboBabo";
                        return;
                    }
