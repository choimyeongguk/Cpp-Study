...
void solve(ll testcase){
    ...
    vpll sorted(N);
    for (ll i=0; i<N; i++) sorted[i] = { i+1, true };

    set<vpll> visit; visit.emplace(cake);
    queue<vpll> q; q.emplace(cake);
    ll cnt = 0;
    while (!q.empty()) {
        ll qsz = q.size();
        // debug(cnt)
        for (ll i=0; i<qsz; i++) {
            vpll cur = q.front(); q.pop();
            // io << "ori : ";
            // for (auto e: cur) io << e.first << (e.second?'+':'-') << " ";
            // io << "\n";
            if (cur == sorted) { io << cnt; return; }
            for (ll j=1; j<=N; j++) {
                reverse(cur.begin(), cur.begin()+j);
                for (ll k=0; k<j; k++) cur[k].second = !cur[k].second;
                // io << "      ";
                // for (auto e: cur) io << e.first << (e.second?'+':'-') << " ";
                // io << "\n";
