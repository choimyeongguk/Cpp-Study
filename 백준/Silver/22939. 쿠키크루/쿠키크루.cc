...
void solve(ll testcase){
    ll N; io >> N;
    vector<string> M(N); for (auto& e: M) io >> e;
    pll from, to;
    vector<vpll> topping(4);
    for (ll i=0; i<N; i++) {
        for (ll j=0; j<N; j++) {
            switch (M[i][j]) {
                case 'H': from = { i, j }; break;
                case '#': to = { i, j }; break;
                case 'J': topping[0].emplace_back(i, j); break;
                case 'C': topping[1].emplace_back(i, j); break;
                case 'B': topping[2].emplace_back(i, j); break;
                case 'W': topping[3].emplace_back(i, j); break;
            }
        }
    }
    ...
    for (ll i=0; i<4; i++) {
        do {
            ll cost = 0;
            cost += dist(from, topping[i][0]);
            cost += dist(topping[i][0], topping[i][1]);
            cost += dist(topping[i][1], topping[i][2]);
            cost += dist(topping[i][2], to);
            if (Min > cost) Min = cost, ans = i;
        } while (next_permutation(topping[i].begin(), topping[i].end()));
    }
    switch (ans) {
        case 0: io << "Assassin"; break;
        case 1: io << "Healer"; break;
        case 2: io << "Mage"; break;
        case 3: io << "Tanker"; break;
    }
}
