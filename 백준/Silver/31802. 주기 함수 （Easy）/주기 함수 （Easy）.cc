...
void solve(ll testcase){
    ...
    ll a, b; io >> a >> b;
    ll start = (a%N + N) %N, len = b-a;
    debug(start); debug(len);
    ll ans = len/N * prefSum[N];
    len %= N;
    if (start+len-1 < N) {
        ans += prefSum[start+len] - prefSum[start];
    }
    else {
        ans += prefSum[N]-prefSum[start] + prefSum[(start+len-1)%N+1];
        debug((start+len-1)%N);
    }
    io << ans;
}
