...
void solve(ll testcase){
    ...
    for (ll i=0; i<N; i++)
        for (ll j=0; j<=i; j++)
            A[i].emplace_back(io.getLL());
    for (ll i=0; i<N; i++)
        for (ll j=0; j<=i; j++)
            B[i].emplace_back(io.getLL());
    function<vvl(vvl&)> sym = [&N](const vvl& A) {
        vvl ret = A;
        for (ll i=1; i<N; i++)
            reverse(ret[i].begin(), ret[i].end());
        return ret;
    };
    function<vvl(vvl&)> rot = [&N](const vvl& A) {
        vvl ret = A;
        for (ll i=0; i<N; i++)
            for (ll j=0; j<=i; j++)
                ret[i][j] = A[N-i-1+j][N-i-1];
        return ret;
    };
    function<ll(vvl&,vvl&)> diff = [&N](const vvl& A, const vvl& B) {
        ll ret = 0;
        for (ll i=0; i<N; i++)
            for (ll j=0; j<=i; j++)
                ret += A[i][j]!=B[i][j];
    ...
    ll ans = 1e10;
    set<vvl> s; s.emplace(A);
    queue<vvl> q; q.emplace(A);
    while (!q.empty()) {
        vvl cur = q.front(); q.pop();
        ans = min(ans, diff(cur, B));
        for (const auto& nxt: {sym(cur), rot(cur)}) {
            if (!s.contains(nxt)) {
                q.emplace(nxt);
                s.emplace(nxt);
            }
