...
void solve(ll testcase){
    ...
    // HLD
    ll time = 0; vl depth(N+1), sz(N+1), pa(N+1), top(N+1), in(N+1), out(N+1);
    function<void(ll,ll)> dfs1 = [&](ll cur, ll prev) {
    ...
    function<void(ll,ll,ll)> dfs2 = [&](ll cur, ll prev, ll head) {
    ...
    vl arr(N);
    for (auto& [u,v,w]: edges) {
        if (pa[v] == u) swap(u, v); // first가 자식이 되도록
        arr[in[u]] = w;
    }
    SegTree seg(arr);
    ...
    function<ll(ll,ll)> query = [&](ll u, ll v) {
        ll ret = -1;
        while (top[u] != top[v]) {
