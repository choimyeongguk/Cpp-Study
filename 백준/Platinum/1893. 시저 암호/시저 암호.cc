...
ll kmp(string& str, string& pattern) {
    ...
    for (ll i=0,j=-1;i<str.size();i++) {
        ...
        if (str[i]==pattern[j+1]) {
            j++;
            if (j+1==pattern.size()) ret++, j=pi[j];
        }
    }
    return ret;
}
...
void solve(ll testcase){
    string A, W, S; io >> A >> W >> S;
    vector<char> trans(123);
    for (ll i=0; i<A.size(); i++)
        trans[A[i]] = A[(i+1)%A.size()];
    ...
    if (ans.empty()) io << "no solution\n";
    else if (ans.size() == 1) io << "unique: " << ans[0] << "\n";
    else {
        io << "ambiguous: ";
        for (auto e: ans) io << e << " ";
        io << "\n";
