...
struct LazySeg {
    ll n; vl data, tree, lazy;
    LazySeg(vl& arr): n(arr.size()), data(arr), tree(n<<2), lazy(n<<2) {
        init(1, 0, n-1);
    }
    ...
    void push(ll i, ll s, ll e) {
        if (lazy[i] == 0) return;
        ll m = (s+e)>>1;
        if (m-s+1 & 1) tree[i<<1] ^= lazy[i];
        lazy[i<<1] ^= lazy[i];
        if (e-m & 1) tree[i<<1|1] ^= lazy[i];
        lazy[i<<1|1] ^= lazy[i];
        lazy[i] = 0;
    }
...
void solve(ll testcase){
    ...
    dfs(1, 0);
    vl W(N+1); for (ll i=1; i<=N; i++) io >> W[i];
    vl arr(N);
    for (ll i=1; i<=N; i++) arr[in[i]] = W[i];
    LazySeg seg(arr);
    ll op, x, y;
    while (M--) {
        io >> op;
        switch (op) {
            case 1:
                io >> x;
                io << seg.query(in[x], out[x]-1) << '\n';
                break;
            case 2:
                io >> x >> y;
                seg.update(in[x], out[x]-1, y);
                break;
        }
    }
}
