...
void calculate_pi(vl &pi, deque<ll> &s) {
    pi[0]=-1;
...
bool kmp(vl& str, deque<ll>& pattern, vl& pi) {
...
void solve(ll testcase){
    ll N, K; io >> N >> K;
    ll M; io >> M;
    vl crit(M); for (auto& e: crit) io >> e;
    vvl str(N-1);
    for (auto& s: str) {
        io >> M; s.resize(M);
        for (auto& e: s) io >> e;
    ...
    ll idx = 0;
    deque<ll> pat; pat.push_back(-1);
    for (ll i=1; i<K; i++)
        pat.push_back(crit[idx++]);
    while (idx < crit.size()) {
        pat.pop_front();
        pat.push_back(crit[idx++]);
        vl pi(pat.size()); calculate_pi(pi, pat);
        bool flag = true;
        for (auto& s: str) {
            if (!kmp(s, pat, pi)) {
                flag = false;
