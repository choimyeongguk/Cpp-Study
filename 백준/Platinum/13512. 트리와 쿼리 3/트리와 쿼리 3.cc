...
struct HLD {
    ll n, time = 0;
    vvl& G; vl depth, sz, pa, top, in, out, seq;
    HLD(vvl& G, ll root = 1)
        : n(G.size()), G(G),
          depth(n), sz(n), pa(n),
          top(n), in(n), out(n), seq(n) {
        dfs1(root, -1);
        dfs2(root, -1, root);
    }
    ...
    void dfs2(ll cur, ll prev, ll head) {
        top[cur] = head, in[cur] = time, seq[time++] = cur;
        bool heavy = true;
        for (auto nxt: G[cur]) {
    ...
    void updateNode(ll u, SegTree& seg) {
        seg.update(in[u]);
    }
    ll queryPath(ll v, SegTree& seg) {
        ll ret = -1;
        while (top[v] != 1) {
            ll tmp = seg.query(in[top[v]], in[v]);
            if (tmp != INF) ret = seq[tmp];
            v = pa[top[v]];
        }
        ll tmp = seg.query(0, in[v]);
        if (tmp != INF) ret = seq[tmp];
        return ret;
    }
};
...
void solve(ll testcase){
    ...
    while (Q--) {
        ...
        switch (op) {
            case 1:
                hld.updateNode(x, seg);
                break;
            case 2:
                io << hld.queryPath(x, seg) << '\n';
                break;
        }
    }
}
