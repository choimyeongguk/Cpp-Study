...
void solve(ll testcase){
    ll N; io >> N;
    vector<string> strs(N); for (auto& e: strs) io >> e;
    vl ord(N); iota(ord.begin(), ord.end(), 0);
    ll K; io >> K;
    ll ans = 0;
    unordered_set<string> magic, nomagic;
    do {
        string pat; for (auto e: ord) pat += strs[e];
        string str = pat+pat; str.pop_back();
        if (magic.contains(pat)) ans++;
        else if (!nomagic.contains(str)) {
            ll cnt = kmp(str, pat);
            if (cnt == K) {
                ans++;
                magic.emplace(pat);
            }
            else nomagic.emplace(pat);
        }
