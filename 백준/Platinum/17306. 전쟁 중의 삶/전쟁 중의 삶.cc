...
struct Trie {
    ...
    void insert(const string& word) {
        ...
        for (auto& c : word) {
            ll tmp = c=='1';
            if (tree[cur].nxt[tmp] == -1) {
                tree[cur].nxt[tmp] = tree.size();
                tree.emplace_back();
                numNode++;
            }
    ...
    ll getAns() {
        ll ans = numNode;
        ll cur = 0, nxt;
        while ((tree[cur].nxt[0]==-1)+(tree[cur].nxt[1]==-1) == 1) {
            ans--;
            nxt = tree[cur].nxt[tree[cur].nxt[0]==-1];
            if (tree[nxt].isEnd) break;
            cur = nxt;
        }
        return ans + 1;
    }
...
void solve(ll testcase){
    ...
    for (ll i=0; i<N; i++) {
        ...
        string str = bitset<50>(A).to_string();
