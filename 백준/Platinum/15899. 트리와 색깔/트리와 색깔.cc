...
struct MergeSortTree {
    ...
    MergeSortTree(vl& arr): n(arr.size()), tree(n<<2) {
        function<void(ll,ll,ll)> init = [&](ll i, ll s, ll e) {
            ...
            tree[i].reserve(e-s+2);
            while (l<lsz && r<rsz)
                tree[i].emplace_back(tree[i<<1][l] < tree[i<<1|1][r] ? tree[i<<1][l++] : tree[i<<1|1][r++]);
        };
        init(1, 0, n-1);
    }
    ll query(ll l, ll r, ll v) { return query(1, 0, n-1, l, r, v); }
    ll query(ll i, ll s, ll e, ll l, ll r, ll v) {
        if (s>r || e<l) return 0;
        if (l<=s && e<=r) return upper_bound(tree[i].begin(), tree[i].end(), v) - tree[i].begin();
        ll m = (s+e)>>1;
        return (query(i<<1, s, m, l, r, v) + query(i<<1|1, m+1, e, l, r, v)) %MOD;
    }
};

void solve(ll testcase){
    ll N, M, C; io >> N >> M >> C;
    vl color(N+1); for (ll i=1; i<=N; i++) io >> color[i];
    vvl G(N+1);
    for (ll i=1; i<N; i++) {
        ll u, v; io >> u >> v;
        G[u].emplace_back(v);
    ...
    dfs(1, 0);
    vl arr(N); for (ll i=1; i<=N; i++) arr[in[i]] = color[i];
    MergeSortTree seg(arr);
    ll ans = 0;
    while (M--) {
        ll v, c; io >> v >> c;
        ans = (ans + seg.query(in[v], out[v]-1, c)) %MOD;
    }
    io << ans;
}
