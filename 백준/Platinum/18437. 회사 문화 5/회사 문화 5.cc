...
struct LazySeg {
    ll n; vl tree, lazy;
    LazySeg(ll n): n(n), tree(n<<2), lazy(n<<2, -1) {}
...
void solve(ll testcase){
    ll N; io >> N;
    vvl G(N+1); for (ll i=1; i<=N; i++) G[io.getLL()].emplace_back(i);
    ...
    while (M--) {
        ...
        switch (op) {
            case 1:
                seg.update(in[i]+1, out[i], false);
                break;
            case 2:
                seg.update(in[i]+1, out[i], true);
                break;
            case 3:
                io << seg.query(in[i]+1, out[i]) << '\n';
                break;
        }
    }
}
